...
%token DO IF WHILE 
%token LET LOCAL CONST GLOBAL STRUCT TYPE
...
structitm:
	  LPAREN SYMBOL typedecl RPAREN
		{Structitem( $2, $3, Conststm( Nilconst ), false )}
	| LPAREN SYMBOL AT typedecl RPAREN
		{Structitem( $2, $4, Conststm( Nilconst ), true )}
	| LPAREN SYMBOL typedecl value RPAREN
		{if isConstStm $4 then Structitem( $2, $3, $4, false )
		    else errorAndDie "Tried to use a non-constant default!"}
	| LPAREN SYMBOL AT typedecl value RPAREN
		{if isConstStm $5 then Structitem( $2, $4, $5, true )
		    else errorAndDie "Tried to use a non-constant default!"}
	;
