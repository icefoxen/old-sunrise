...
let rec hasCompatiblePointer pkg typ1 typ2 =
  if isPointer typ1 && isPointer typ2 then

    let t1 = getPtrval typ1
    and t2 = getPtrval typ2 in
      hasCompatibleType pkg t1 t2
  else if (not (isPointer typ1)) && (not (isPointer typ2)) then
    hasCompatibleType pkg typ1 typ2
  else
    false
...
(* Note that the order of typ1 and typ2 IS significant, because
   ...
   ...okay.  I think the problem is we really have two different kinds of 
   arrays.  We have fixed-length static arrays, and pointers to dynamic arrays.
   Static arrays: Must be initialized to an array of the same type, have a 
   fixed length, cannot be nil, and can be initialized from a single value
   Dynamic arrays: Can be initialized to an array of the same type, or nil, 
   have a length of -1, and cannot be initialized from a single value.
*)

and hasCompatibleArray pkg typ1 typ2 =
  let saneDynamicArray pkg t1 t2 =
    let a = resolvePtrval typ1
    and b = resolvePtrval typ2 in
    let al = getArrayLen a
    and bl = getArrayLen b in
    let at = resolveCustomType pkg (getArrayType a)
    and bt = resolveCustomType pkg (getArrayType b) in
      if (al = -1) then
	(hasCompatibleType pkg at bt) || (bt = Nulltype)
      else
	(al = bl) && (hasCompatibleType pkg at bt)
	

  and saneStaticArray pkg t1 t2 =
    let al = getArrayLen t1
    and bl = getArrayLen t2 in
    let at = resolveCustomType pkg (getArrayType t1)
    and bt = resolveCustomType pkg (getArrayType t2) in
      if (at = Nulltype) or (bt = Nulltype) then
	false
      else if (al < 1) || (bl < 1) then
	errorAndDie ("Array " ^ (typestmt2str t1) ^ " or " ^
		     (typestmt2str t2) ^ " is not sane!")
      else
	(hasCompatibleType pkg at bt) && (al = bl)
  in
    if isPointer typ1 then
      saneDynamicArray pkg typ1 typ2
    else
      saneStaticArray pkg typ1 typ2
...
and hasCompatibleArray pkg typ1 typ2 =
  ...
  if ((isSaneArray pkg typ1) && (isSaneArray pkg typ2)) then
	...
	(typestmt2str typ1) (typestmt2str typ2);
	 ...
	 But that makes a leeeetle infinate loo.
      *)
      if a = b then  
	true
      else if at = bt then
	if (al = bl) or (al = -1) or (bl = -1) then begin
	  print_endline "Kweh!";
	  true
	end else
	  false
