...
(* Analyzes all the types, structs, globals, consts, and functions. 
   ...
   XXX: Make pointers and addresses work!
   ...
   (let (foo :int) (bar :^:int) (bop :^:int))
   (set bar (& foo)) (set bop bar) return bop
   ---Bah, it's simple.  Assign statement checks to see if the lvalue is
   a pointer and the rvalue is /anything/ but a global, const, heap, or
   another pointer.
   Well, that makes pointers to local wossnames entirely illegal, which
   may or may not be what we want.
   Probably not.  Let's just check what we can, hey?
   XXX: Double-check arithmatic conversions...  (set intvar (+ 10 95.2))
   vs (set intvar 95.2)
   XXX: WTF happens if you try to treat an inner function like a closure,
   and return it?
*)
...
let analyze syntree pkg = 
  ...
  *)
  and analyzeVar name defval =
	...
	let msg = Printf.sprintf "Variable %s's default value is of the\n wrong type!  It should be %s\n" name (typestmt2str deftype) in
	  errorAndDie msg
  ...
  and analyzeStmt stm =
	...
	Opexpr( op, stmlst ) -> analyzeOpexpr op stmlst
	 ...
	 struct *)
      ...
      | Ptrstm( stm ) ->
	  let stmtype = analyzeStmt stm in
	    if Symtbl.isPointer stmtype then
	      Symtbl.getPtrval stmtype
	    else
	      errorAndDie ("Tried to derefernce a " ^ (typestmt2str stmtype))
      | Addrstm( stm ) ->
	  match stm with
	      Varstm( _ )
	    | Aref( _ )
		...
		let stmtype = analyzeStmt stm in
		  Ptrtype( stmtype )
	    | _ -> 
		errorAndDie ("Tried to get the address of something that isn't a var, array or struct!")
